<?php 
	session_start(); 
	include '../connection.php';
	if (isset($_POST['submit'])){
		date_default_timezone_set('Asia/Kathmandu');
		$user=$_SESSION['Username'];
    	$v1=date('h:i a');
		$v2=mysqli_real_escape_string($con,$_POST['to']);
		$v3=mysqli_real_escape_string($con,$_POST['subject']);
		$v4=mysqli_real_escape_string($con,$_POST['message']);	
		$v5=mysqli_real_escape_string($con,$_POST['cc']);
		$rand=rand(000,999);
		
		if($v2==$user){
			echo "You cannot send email to yourself.";
		}

		else{
			$insert="INSERT INTO message(`date`,`Frm`,`To`,`Message`,`cc`,`Subject`,`St_read`,`reply`,`prime_mail`) values('$v1','$user','$v2','$v4','$v5','$v3','false','$rand',1)";
			$query=mysqli_query($con,$insert);
			
			if($query){
				header('location:inbox?success');
			}
			else{
				header('location:inbox?failed');
			}
		}
}
 ?>