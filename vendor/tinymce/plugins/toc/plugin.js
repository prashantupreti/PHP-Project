...
var toc = (function () {
  ...
  var $_21momirhjglr4zot = { create: create };

  var tocId = $_21momirhjglr4zot.create('mcetoc_');
  ...
  var readHeaders = function (editor) {
    var tocClass = $_8i1gfdrgjglr4zot.getTocClass(editor);
    var headerTag = $_8i1gfdrgjglr4zot.getTocHeader(editor);
    var selector = generateSelector($_8i1gfdrgjglr4zot.getTocDepth(editor));
  ...
  var generateTocHtml = function (editor) {
    ...
    return '<div class="' + editor.dom.encode($_8i1gfdrgjglr4zot.getTocClass(editor)) + '" contenteditable="false">' + html + '</div>';
  };
  var generateTocContentHtml = function (editor) {
    ...
    html += generateTitle($_8i1gfdrgjglr4zot.getTocHeader(editor), global$2.translate('Table of Contents'));
    for (i = 0; i < headers.length; i++) {
      h = headers[i];
      h.element.id = h.id;
  ...
  var register = function (editor) {
    editor.addCommand('mceInsertToc', function () {
      $_damfuvrcjglr4zoe.insertToc(editor);
    });
    editor.addCommand('mceUpdateToc', function () {
      $_damfuvrcjglr4zoe.updateToc(editor);
    });
  };
  var $_et5g9irbjglr4zoe = { register: register };

  var setup = function (editor) {
    var $ = editor.$, tocClass = $_8i1gfdrgjglr4zot.getTocClass(editor);
  ...
  var $_4eeow7rijglr4zot = { setup: setup };

  var toggleState = function (editor) {
    return function (e) {
      var ctrl = e.control;
      editor.on('LoadContent SetContent change', function () {
        ctrl.disabled(editor.readonly || !$_damfuvrcjglr4zoe.hasHeaders(editor));
      });
    };
  };
  var isToc = function (editor) {
    return function (elm) {
      return elm && editor.dom.is(elm, '.' + $_8i1gfdrgjglr4zot.getTocClass(editor)) && editor.getBody().contains(elm);
    };
  };
  ...
  var $_92ipkjrjjglr4zot = { register: register$1 };

  global.add('toc', function (editor) {
    $_et5g9irbjglr4zoe.register(editor);
    $_92ipkjrjjglr4zot.register(editor);
    $_4eeow7rijglr4zot.setup(editor);
  });
  function Plugin () {
  }
